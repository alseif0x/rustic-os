...
For nontrivial work, use the project rustic-orchestrator skill and delegate bounded implementation or independent evidence/review when useful. Start with docs/WORK-STATE.md and the active issue instead of replaying the full history. Codex uses Opus 5 high as root and DeepSeek V4.1 Flash high as worker through the installed Codex Router; explorer/tester/researcher retain Luna max and reviewer retains Astra low. Native Claude Code keeps its separate Fable medium root, Opus high explorer/worker/tester/researcher and Fable low reviewer. Preserve these user-selected roles and efforts. Use at most two concurrent children, minimal context, one writer per area and one owner for shared build/VM tests. Do not spawn every role mechanically. Simple edits and questions may stay with the root. See docs/ORCHESTRATION.md for loading, evidence and continuation rules; explicit user instructions take precedence.
