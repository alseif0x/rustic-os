...
The goal is a practical computer where people can work directly, collaborate with an agent, delegate and take over without losing their work. Agent support is part of the architecture; running a model is optional. Manual use and recovery must work without one. Structured actions do not require screen interpretation, while a graphical interface and visual automation remain complementary options.
...
| [Live execution control](docs/FILE-ACTIVITY.md) | Query an active file execution and request cancellation while disk I/O is pending; recover the durable result separately |
| [Operation lifecycle v2](docs/FILE-LIFECYCLE.md) | Stable operation IDs, proven terminal states and minimal cancellation acknowledgements with independent permissions |
| [Native profile selection](docs/FILE-NEGOTIATION.md) | Explicit lifecycle version, reviewed contract digests, enforced limits and the current IPC responder |
| [Native capability discovery](docs/DISCOVERY.md) | Implemented methods for the mounted volume and enforced bounds, cross-checked against behaviour and unchanged after reboot |
...
[Lifecycle v2](docs/FILE-LIFECYCLE.md) adds typed inspection of prepared, live and
...
[Workspace replacements and completed operations](docs/FILE-OPERATIONS.md) provide scoped retry keys, immutable historical receipts and lookup after a lost response or restart. [Explicit admissions](docs/FILE-ADMISSION-API.md) add durable preparation and caller-controlled execution. [Live execution control](docs/FILE-ACTIVITY.md) lets authorized clients query and request a stop while disk I/O is pending; the durable outcome is recovered separately. Pending work never resumes automatically after restart. [Scheduled execution](docs/FILE-SCHEDULING.md) now returns control to its initiating client, with one active publication and bounded pending work. [Scheduled failure tests](docs/FILE-SCHEDULING-FAILURES.md) cover selected late stops, response loss, service restart, device errors and combined pressure. [Native authority tests](docs/FILE-SCHEDULING-AUTHORITY.md) verify intervening human edits, scoped permissions and prompt revocation refusals. [Coherent observation](docs/FILE-OBSERVATION.md) follows prepared, live and retained work through one typed query and a stable admission ID. [Retained prevention causes](docs/FILE-PREVENTION.md) distinguish cancellation, version conflict and lost authority on explicitly migrated volumes; the explicitly selected observation profile 2 exposes those causes through the SDK and terminal while preserving profile 1. The [service-v2 lifecycle](docs/FILE-LIFECYCLE.md) now preserves stable operation identity and returns minimal cancellation acknowledgements under independent authority. [Native profile selection](docs/FILE-NEGOTIATION.md) checks the lifecycle version, reviewed descriptor digest and enforced limits on the current IPC connection. **Next:** complete the applicable shared failure vectors in [#47](https://github.com/alseif0x/rustic-os/issues/47), then integrate scoped discovery and the deterministic mission in [#22](https://github.com/alseif0x/rustic-os/issues/22). [Browser feasibility](docs/architecture/browser-feasibility.md) starts early to identify application/runtime needs. The [measurement harness](docs/MEASUREMENTS.md) evaluates resource cost. This remains a bounded prototype with shared two-record retention and no general delegation or production filesystem guarantees.
