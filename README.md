...
[Recoverable native replacements](docs/FILE-RECOVERY.md) now retain atomic file/version receipts, reject conflicting retries and preserve results across service/VM restart. [Interruptible owner control](docs/FOREGROUND-CONTROL.md) keeps supervision available during real submitted I/O and verifies recovery across restart/reboot. **Next:** complete service-v1/workspace semantics in [#12](https://github.com/alseif0x/rustic-os/issues/12)/#22, complete the remaining takeover cases after the [native client/helper mission](docs/AUTHORITY.md) in [#13](https://github.com/alseif0x/rustic-os/issues/13), and measure the topology in #20. This remains a bounded prototype without general delegation or production filesystem guarantees.
