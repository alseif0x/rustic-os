...
[Native `files.read`](docs/FILES-READ.md) connects one logical operation to the real file service, shell and shared SDK. Stable workspace/resource references identify files without granting authority; reads pin the version, verify the returned range's SHA-256 and discard incomplete output. Its shared host/native range fixtures cover this operation; the full eight-operation catalog remains planned.
...
The kernel does not depend on MCP or a model. Native file/supervision services support the manual terminal, and the shell and deterministic client share the typed `files.read` SDK path. The first eight [shared service contracts](docs/SERVICE-CONTRACTS.md) have checked schemas and generated descriptors; only the read operation has this native logical binding. The broader transaction, discovery, tool and agent layers remain roadmap work. [Agent integration](docs/architecture/agent-integration.md) records the adapter design and open experiments.
...
[Recoverable native replacements](docs/FILE-RECOVERY.md) retain atomic file/version receipts, reject conflicting retries and preserve results across service/VM restart. [Interruptible owner control](docs/FOREGROUND-CONTROL.md) keeps supervision available during real submitted I/O, and [stable reads](docs/FILES-READ.md) expose verified bytes through the same SDK used by native clients. **Next:** complete per-workspace mutation/retry semantics and the remaining service-v1 methods in [#12](https://github.com/alseif0x/rustic-os/issues/12)/[#22](https://github.com/alseif0x/rustic-os/issues/22), and complete the remaining takeover cases after the [native client/helper mission](docs/AUTHORITY.md) in [#13](https://github.com/alseif0x/rustic-os/issues/13). The [measurement harness](docs/MEASUREMENTS.md) provides compatible baselines for evaluating those changes. This remains a bounded prototype without general delegation or production filesystem guarantees.
