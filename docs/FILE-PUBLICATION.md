...
This is the filesystem prerequisite for asynchronous cancellation in #12/#13/#43. `rustic-fs` can prepare a replacement and advance its publication one settled disk command at a time. The existing synchronous file-service APIs drive that same implementation to completion. This increment does **not** expose `operations.cancel`, acknowledge durable queued/running work, or make the file-server event loop nonblocking.
...
| `Settling` | `TooLate` | The header write succeeded; continue through the final flush. |
...
Any failed write/flush is conservatively uncertain, including a failure before the header. Failure is not proof that the device made no change. Cancellation cannot relabel that failure as rollback. Dropping a healthy preparation before publication releases the live writer; dropping during final settlement leaves it fenced. A forgotten publication guard also leaves the writer fenced. Fencing is established before I/O, including an adapter panic that unwinds on the host; successful settlement or known safe cancellation releases it.
...
The service still needs to own and poll submitted disk requests without blocking control IPC. Its cancellation authority must remain distinct from read/inspect authority, with revalidation before publication and explicit handling of revocation while I/O is outstanding. The controller must drain or cancel an admitted command before claiming that publication was prevented.
