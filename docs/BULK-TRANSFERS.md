...
- **Still required before claiming throughput:** memory cost per transfer slot and queue measured rather than estimated (the constants are 40 bytes per operation today; the queue entry is `24 + PAYLOAD`), and a saturation case with a stopped reader.
