...
- **Control progress while work is in flight, measured on the same trunk run:** an owner command issued while a client holds write pressure completes in **0.069 s**; the worst owner command while a service is deliberately stalled is **0.84 s** against a 2 s tripwire; an owner interrupt during pending I/O takes **0.21 s**; a revocation takes **0.021 s**. These are existing acceptance measurements, not new ones, and they are the evidence that bulk data does not take the control path away.
- **Control progress during a large V7 streamed write:** the V7 fault run (`python3 tools/v7_faults_test.py`) times an owner `INFO` before every 64th 40-byte chunk of two 512 KiB tracked writes: 206 samples each, maximum 1 tick (under 20 ms), median 0 ticks (guest ticks of 10 ms), with `free_frames` and `heap_pages` unchanged throughout; an owner revocation 400 chunks into a 512 KiB write took at most 1 tick (under 20 ms) across three runs. The shell sends each query between chunks, so this shows the owner path is free while a transfer is open, not a query competing with an in-flight disk command. See [memory and control latency](FILES-V7-WRITES.md#memory-and-control-latency).
- **Measured memory and reclamation on the trunk:** `RUSTIC PROCESS_MEMORY` reports the process manager at **9,816 bytes** on the reference profile; the manager owns the channel queues, and the queue share is arithmetic on the constants (8 channels x 2 directions x 2 messages x (24 + 64) = 3,584 bytes) inside that measured total. After the IPC fixture, `RUSTIC IPC` reports `channels=0 handles=0 free_before=52006 free_after=52006`: every channel and handle is returned.
- **Saturated data with a stopped reader is already executed:** the terminal acceptance's saturation case retains both staging slots, makes one client stop draining its replies, arms real pending I/O (`hold-io`, `io-status held=1`) and then requires that the owner console still runs, a live execution observation still answers, a public stop is accepted and not lost, `mem` still reports `pending_io=1`, and a further write is refused with `Busy` rather than blocking. That is this issue's "bounded memory and control progress under saturated data traffic and a stopped reader" bullet.
- **Storage accounted, with the measured part named:** the queue entry is `24 + PAYLOAD` bytes and is inside the measured 9,816-byte manager; a client buffer for one message is `MAX_MESSAGE` = 88 bytes today; the entry stack is reported as 20,480 bytes (`RUSTIC PROCESS_MEMORY`) and the user stack is the bounded 64 KiB configuration. High-water stack use is not measured in this repository (`docs/MEASUREMENTS.md` says so), so the accounting is a bound, not a measurement, and the applications running without faulting is the only execution evidence for it.
