...
The allocator metadata is derived from the frame-bitmap address budget, so [#48](https://github.com/alseif0x/rustic-os/issues/48) doubles it to 131,072 B (two 64 KiB bitmaps over a 2 GiB budget). The stored `r0-initial.json` budget for `allocator_metadata_bytes` remains 65,536 B as the historical calibration; comparing a current build against it therefore reports a regression by design until #20 records a re-baselined budget with fresh native measurements. This is a reviewed constant rather than measurement noise; the memory profile suite records the cost per profile (`artifacts/` is gitignored, so it is regenerated rather than linked).
