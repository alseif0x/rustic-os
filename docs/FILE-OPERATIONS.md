...
The filesystem provides [publication steps and local cancellation boundaries](FILE-PUBLICATION.md), shared by synchronous and polling adapters. The [owner-control integration](FILE-CONTROL.md) receives revocation while this logical replacement has pending I/O and stops before header publication when possible. Clients still receive completed results only; ordinary file requests wait until the active publication finishes.
...
The combined recovery mission now has 19 groups across 38 VM boots. Six workspace-operation groups exercise the real discarded IPC reply, independent workspace namespaces, read-versus-inspect authority, later edits/deletion/recreation, explicit service restart, VM reboot, quota/epoch behavior and five data/receipt/metadata/header/final-flush EIO boundaries. Four [owner-control groups](FILE-CONTROL.md) add revocation during pending publication. Native tests use disposable disks and an independent format/content oracle. Pure filesystem tests additionally cut migration and publication writes/flushes, including torn sectors and both visible/durable disk views; service and SDK tests challenge revocation and malformed fragments.
