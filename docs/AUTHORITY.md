...
- Owner control during an admission publication is not implemented yet (the publication runs to settlement inside the request), so revocation cannot interrupt one and the `AuthorityLost` cause is not produced on V7. Retention maintenance stays `Busy` while any admission is unresolved.
