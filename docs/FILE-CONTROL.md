...
Review covers separate storage/authority/transport ownership, directed crate dependencies, terminal receipt visibility and outstanding-command lifetimes. No new unsafe code, external dependency, kernel policy, wire opcode or on-disk format is introduced. Ordinary reads, legacy file mutations and storage administration still use synchronous I/O. Public cancellation, durable admission IDs, retained cancellation records, current-operation discovery and full native/function/MCP conformance remain subsequent work.
