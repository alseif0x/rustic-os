...
Each active device request has a 500-tick deadline (nominally five seconds) and a fallback after 5,000,000 consecutive polls without tick progress. The [block budget rationale](BLOCK.md) records the observed CI FLUSH failures behind this choice. The deadline begins at device submission; completed results remain until collection/close/exit. Timeout/protocol failure fences reuse; reset must be read back before DMA frames can be freed. The service may reopen the same fixed device only after reset-confirmed shutdown and matching geometry. A failed reset quarantines the DMA allocation and PCI claim. A restarted process needs a fresh owner-bound handle; old handles and requests never become authority for it. Owner control can execute while I/O is pending; this is bounded polling on one CPU, not an interrupt-driven or SMP storage stack. The five deliberately unnotified requests in `block-user-faults` consume about 25 guest seconds; direct and isolated suites give that scenario 45 host seconds including setup and control progress.
