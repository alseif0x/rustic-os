...
The [service control integration](FILE-ADMISSION-CONTROL.md) now makes admission and terminal metadata writes pollable, binds fresh execution authority and persists terminal cleanup after owner revocation. The remaining public integration must define a separate cancellation right and bounded accepted/result framing, then prove real IPC response loss, revocation, restart and selected native I/O failures. These storage states do not add another live catalog operation or establish full native/function/MCP conformance, physical power-loss guarantees or automatic execution after restart.
