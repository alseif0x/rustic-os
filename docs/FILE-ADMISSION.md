...
The next service increment must make admission and terminal metadata writes pollable under owner control, bind fresh execution/cancellation authority, and define bounded accepted/result framing around the new identity. Only then can the public asynchronous/cancel profile be implemented and tested with real IPC response loss, revocation, restart and selected native I/O failures. These storage states do not add another live catalog operation or establish full native/function/MCP conformance, physical power-loss guarantees or automatic execution after restart.
