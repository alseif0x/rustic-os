...
The recovery-test mode runs the same reviewed driver against the exact candidate ELF across 38 boots. It exports recovery.json (64 KiB) and files.bin (89,088 bytes). Its boot-stage wall limit is sixteen times the per-interaction timeout plus 30 seconds, covering the bounded sequence of fresh VMs; no VM, disk or path is selected by candidate input. The existing CPU, memory, disk, network and container ownership limits remain in force. [Legacy recovery](FILE-RECOVERY.md), [workspace operations](FILE-OPERATIONS.md) and [owner-control races](FILE-CONTROL.md). The reviewed infrastructure includes both read and replacement fixture corpora; rebuild it with prepare after changing those drivers or fixtures.
