...
For an indefinite stall, `restart files` kills old utilities and the file service, checks pending kernel I/O and requires successful reap before remount/fresh owner bindings. If device work is still pending it returns Busy and retains the old service PID for a later retry. Old actors and contexts never restart automatically. The old retained record remains conservative about effects; newly issued sessions get a fresh service binding.
...
**Remaining boundary:** this test stalls before device admission. It does not prove the restart Busy/retry path with an in-flight write, complete asynchronous admitted-device takeover, or full service-v1 operation/reference/regrant semantics. Foreground file commands, grant setup, policy loading and service startup/remount still use bounded synchronous exchanges; an unexpected stall during those calls can delay the single shell/supervisor path. The SDK poisons a timed-out binding so later calls cannot consume an old reply as a new result. Ctrl-C still cancels input editing only. Converting those remaining waits and injecting admitted-I/O failure are the next #13 work. #20 must measure the resulting topology repeatedly. Review is by the implementing agent and automated checks, without an independent audit.
