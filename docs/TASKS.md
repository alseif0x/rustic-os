...
The candidate bytes are not part of the child's image: they live in memory it reserves while it needs them. `TASKS_OWNER_EDIT` and `TASKS_OWNER_BEGIN` reserve one page through `rustic_sdk::memory::Heap` and allocate the bounded document's block inside it, and the collection assembles the plan directly in those bytes — one buffer, no copy of its own. The block is released and the page unmapped as soon as the client holds nothing the bytes are for: when an apply concludes (a success or one of the conclusive refusals), when a chunk that fails the document rules leaves the client idle, and between two hand-offs. So `apps/utility` is the first product application whose heap usage is visible from the ordinary control path: this process maps no page at all while it is idle or finished, one page while a plan is announced, collected or ready, and none again after `tasks-apply`. A reservation or allocation that is refused answers `106`, maps nothing, retains nothing and leaves the phase exactly as it was; the [SDK heap](SDK.md) owns the mapping rules and is where the `unsafe` for them lives, while this client stays `#![forbid(unsafe_code)]`.
...
| `TASKS_HEAP_STRESS` | `[error, peak_pages, full_observed, pages_after_release, page_limit]` — grows a private heap until the kernel refuses with `Full`, releases it and reports; `error` is `0` only when the peak reached the per-process budget and the pages mapped afterwards equal those mapped before |
