...
The recovery-test driver runs 18 actual VM boots using the candidate's native ELFs. It checks a discarded native IPC reply, reauthorization after file-server restart, replay after human edits, stale versions, key conflicts, zero-length replacement, receipt quotas, durable epoch rotation, separate-VM lookup and legacy upgrade. Five cases inject QEMU EIO at data, receipt-bank, object-metadata, publication-header and final-flush boundaries. Each reports Uncertain in the guest, remounts the service, abruptly terminates only that owned VM and verifies the recovered pair in a second VM. Independent Python CRC/record/content decoding corroborates the result. QEMU's [blkdebug documentation](https://www.qemu.org/docs/master/devel/testing/blkdebug.html) describes the reference fault mechanism; these EIO faults are controlled by the host. Two additional [admitted-I/O cases](FOREGROUND-CONTROL.md) hold kernel completion observation after a real first-data-write or final-publication-flush submission, interrupt the owner reply, drain during explicit restart and verify old/unknown or new/committed state in a second VM. The bounded guest diagnostic requires trusted supervisor authority and the sdk-test build configuration.

This covers selected guest I/O failures and process/VM termination, not physical power failure, every device-cache behavior or successful persistence after arbitrary acknowledged-sector loss. Full logical service-v1 conformance, SHA-256 result receipts, workspace-generation namespaces, general cancellation and broader authority/takeover cases remain assigned to #12/#13/#22/#43.
...
[CI run 34529363684](https://github.com/alseif0x/rustic-os/actions/runs/34529363684) returned an unexpected `Uncertain` for the initial post-rotation replacement at revision `eb1be0f`. The same revision passed the separate isolated recovery mission, and the preceding local direct and isolated missions passed. The failed direct run remains evidence of an unresolved intermittent failure tracked in [#46](https://github.com/alseif0x/rustic-os/issues/46); later successes alone do not identify or repair its cause. Its original UART log cannot distinguish a device failure from a COMMIT transport or receipt-validation failure, and that runner deleted its temporary disk.
