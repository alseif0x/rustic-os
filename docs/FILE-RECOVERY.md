...
The token is lineage[16], epoch u64, key u64, little endian. CHUNK, ABORT and COMMIT are shared with ordinary transfers. A tracked COMMIT response has id=object, version=committed version, arg=length and a 40-byte payload containing token[32] plus previous version u64. Additional errors are Unsupported=24, Lineage=25, ExpiredEpoch=26, OutcomeUnknown=27 and IdempotencyConflict=28. Rights are read=1, write=2, inspect=4. Private admin grant word 7 carries the trusted subject; private opcode 36 rotates retained evidence. These fields are absent from ordinary file requests.
