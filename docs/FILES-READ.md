...
path lookup/stat, untracked mutation or legacy-file route, so use canonical
workspace and resource IDs returned by `rustic-volume seed7` with `read-ref`.
Its only mutation is the streamed profile-2 tracked replacement described in
[V7 tracked writes](FILES-V7-WRITES.md). Its
...
write-profile acceptance (`python3 tools/v7_write_test.py` is).
