...
Implemented as the bounded storage foundation for the [native terminal](TERMINAL.md), with separate pure format/policy crates, SDK clients and a ring-3 file-server application. [Legacy recoverable replacements](FILE-RECOVERY.md) add atomic retained receipts. [Stable bounded reads](FILES-READ.md) and [workspace replacements/completed operations](FILE-OPERATIONS.md) connect three logical methods through the shared shell/client SDK. [Original H1 acceptance](FILE-FOUNDATION-ACCEPTANCE.md) belongs to #12; additional H2 execution/cancellation belongs to #47. #22/#43 retain complete service-v1 conformance; the eight-operation surface is not advertised as implemented.
...
REFERENCES resolves a selected directory and resource to canonical lineage/workspace/object identities after scope checks. READ_OPEN observes the version, full size, range SHA-256 and retry epoch; READ_CHUNK rechecks authority and that pinned version for each bounded portion. These requests add no persistent read lease or staging slot. The SDK collects into caller-owned memory, verifies the complete range and clears output on failure. The hash covers returned bytes, and observing the epoch does not grant receipt-inspection authority. See [the exact wire layout and read contract](FILES-READ.md).

The separate private admin channel accepts bounded grant/revoke/detach/status/derive words, never ordinary client file requests. [Session roots and helper subsets](AUTHORITY.md) bind derived grants to their revocation context. Revocation fences all members atomically between requests and reports discarded staging and whether the volume requires recovery. Queued replies are discarded when replacing a grant; closed endpoints are closed on both sides so channels are reusable. One request per client per dispatch pass and an owned reply slot avoid allowing a full output queue to stop unrelated clients.
...
Pure tests cover names, directory constraints, quota recovery, stale versions, read-only roots, malformed metadata, checksum rejection and torn writes/flushes. Native terminal acceptance adds real disk I/O, scopes, manual recovery, service restart and separate VM boots with independent disk decoding. [Legacy recovery](FILE-RECOVERY.md), [stable reads](FILES-READ.md) and [workspace operations](FILE-OPERATIONS.md) document their respective fault and conformance profiles. General asynchronous lifecycle, cancellation/events and the complete service-v1 surface remain open in #47/#15/#22/#43. #13 retains authority obligations. Independent workspace retention and general delegation need separately justified scope; they are not prerequisites for closing the bounded H1 file foundation.
