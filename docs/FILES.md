...
Pure tests cover names, directory constraints, quota recovery, stale versions, read-only roots, malformed metadata, checksum rejection and torn writes/flushes. Native terminal acceptance adds real disk I/O, scopes, manual recovery, service restart and separate VM boots with independent disk decoding. [Legacy recovery](FILE-RECOVERY.md), [stable reads](FILES-READ.md) and [workspace operations](FILE-OPERATIONS.md) document their respective fault and conformance profiles. General asynchronous lifecycle, cancellation/events, independent workspace retention, generic helper delegation and the complete service-v1 surface remain open in #12/#13/#22/#43.
