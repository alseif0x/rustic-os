...
contract; its eight schemas and digests are unchanged. The subsequent
[native lifecycle selector](FILE-NEGOTIATION.md) checks the reviewed v2 profiles
on the current IPC connection. General registry discovery remains separate work.
...
The live-control increment adds `activity-check` for the declared correspondence itself and `activity-native --evidence RECOVERY_JSON` for the eight native live-control groups. It maps observed native facts onto the reviewed `operation` type and rejects a mapping that would report false success, a rollback that did not happen, or a result under stale authority: a volatile observation never becomes a terminal state, `settling` maps to `reconciling`/`unknown` rather than claiming `none`, an uncertain attempt stays `reconciling` whatever the retained record says, and refused live requests map to contract errors instead of an operation object. Prepared admission is not scheduled work, and success requires a matching case-local receipt. The mapping also records a real divergence: the native profile identifies pre-terminal work by its admission and a committed result by its completed operation, so one stable `operation_id` across the whole lifecycle does not yet exist. This is a correspondence check over recorded evidence, not an implementation of the live `operations.get`/`operations.cancel` lifecycle on the guest; `operations.cancel` remains `specified_not_implemented`. See [live execution control](FILE-ACTIVITY.md).

The discovery increment adds `capabilities-check` for the declared shape and `capabilities-native --evidence TERMINAL_JSON` for a real run. The file service reports which catalog methods it implements on the volume it has actually mounted, with the bounds it enforces; the validator checks catalog identity/order, mounted-format support, registry limitations and complete native read evidence. Format support does not itself prove a replacement executed; the recovery mission supplies that evidence. `capabilities.list` and `capabilities.describe` remain `specified_not_implemented`: the v1 registry still needs its service-instance identity, session visibility, pagination and complete descriptors. The separate [v2 native selector](FILE-NEGOTIATION.md) supplies reviewed lifecycle digests and the current IPC responder within its explicitly bounded profile. See [native capability discovery](DISCOVERY.md).
