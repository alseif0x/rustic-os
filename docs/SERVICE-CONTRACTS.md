...
Before acknowledging `queued` or `running`, reserve and durably record a bounded operation slot. A completed file replacement must commit data, version and its receipt as one recoverable local transition. `succeeded` is emitted only after that transition meets the documented persistence guarantee. A metadata/data write followed by an unrelated best-effort log is insufficient. #12 must select the disk format and interrupted-write/recovery mechanism; #22 must verify effect/receipt recovery before advertising this guarantee. Inability to reserve evidence is `quota_exceeded` before changing the file.
...
The existing [IPC](IPC.md) has a 64-byte payload; the [block driver](BLOCK.md) transfers 512-byte sectors. No complete inline JSON request or sector is assumed to fit. The service envelope is a logical contract; IPC supplies authenticated sender and transport correlation. #44 owns exact syscall/transfer encoding and numbers. Service protocol IDs/versions remain separate from the IPC framing; resource IDs resolve to service objects while kernel handles remain owner-checked native values and are never synthesized from strings.

For the first block bridge, compare a copied-buffer call (one validated 512-byte sector into kernel-owned staging) with a bounded chunk protocol over existing IPC. Prefer the copied-buffer baseline if its scheduling/lifetime tests pass: it avoids at least eight payload chunks plus control exchanges per sector. Record this as a candidate, not an implemented syscall. Never expose DMA frames to file clients or retain an unpinned user pointer. Read output needs full range validation before consuming completion; a write needs a complete owned snapshot before submission. Define owner/incarnation, read/write/flush rights, geometry, range/overflow, one-in-flight quota, deadline, cancellation and reset-confirmed cleanup. #44 must test the selected behavior with a real ring 3 server and survivor progress.
