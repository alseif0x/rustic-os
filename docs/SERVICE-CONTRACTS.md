...
The live-control increment adds `activity-check` for the declared correspondence itself and `activity-native --evidence RECOVERY_JSON` for the seven native live-control groups. It maps observed native facts onto the reviewed `operation` type and rejects a mapping that would report false success, a rollback that did not happen, or a result under stale authority: a volatile observation never becomes a terminal state, `settling` maps to `reconciling`/`unknown` rather than claiming `none`, an uncertain attempt stays `reconciling` whatever the retained record says, and refused live requests map to `access_denied`/`outcome_unknown` errors instead of an operation object. The mapping also records a real divergence: the native profile identifies pre-terminal work by its admission and a committed result by its completed operation, so one stable `operation_id` across the whole lifecycle does not yet exist. This is a correspondence check over recorded evidence, not an implementation of `operations.get`/`operations.cancel` on the guest; `operations.cancel` remains `specified_not_implemented`. See [live execution control](FILE-ACTIVITY.md).

The discovery increment adds `capabilities-check` for the declared shape and `capabilities-native --evidence TERMINAL_JSON` for a real run. The file service reports which catalog methods it implements on the volume it has actually mounted, with the bounds it enforces; the validator rejects a report that loses the catalog identity or order, contradicts the volume's operation support, overstates the registry contract, or advertises a method the run never exercised. `capabilities.list` and `capabilities.describe` remain `specified_not_implemented`: the guest reports no service instance for discovery and carries no contract digest. See [native capability discovery](DISCOVERY.md).
