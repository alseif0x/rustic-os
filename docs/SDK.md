...
The first product consumer is the tasks-owner child of `apps/utility`: the candidate bytes of the [second native tasks client](TASKS.md) live in a block of a heap it reserves when a hand-off announces an edit and drops when the apply concludes, so a product application's heap rises and falls through the ordinary shell control path instead of a fixture. It uses the public `Heap` API alone — `reserve`, `alloc`, `bytes_mut`, `free`, `Drop` — and keeps `#![forbid(unsafe_code)]`. `apps/sdk-probe` remains the fixture: it is where alignment, reuse, zeroing, the budget refusal and the release are exercised deliberately and recorded, and it is not a product path.
...
An explicitly unbound file client (endpoint token zero) returns `files::Error::Unavailable` before request, submit or read observation reaches IPC. A fresh authorized `rebind` restores admission. This keeps failed service startup distinct from malformed replies or poisoned nonzero bindings; their existing error and uncertainty classifications remain unchanged. Host binding tests run the actual file/RPC code over a safe fake endpoint, while native missions establish real service execution.
