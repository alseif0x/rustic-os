...
Later publication/admission fixtures add the existing original `rustic-fs` and `rustic-file-service` path dependencies to `rustic-block-probe`. This lets the native acceptance application execute the same storage and authority policy as the file server. External versions/checksums and preserved notices remain unchanged; the kernel gains no service or SDK dependency.
...
The native `files.read` binding uses RustCrypto's SHA-256 implementation in `rustic-file-service` and `rustic-sdk`. The workspace pins `sha2 = "=0.11.0"` with default features disabled; Cargo.lock fixes all transitive versions and archive checksums. Application manifest v2 also uses this implementation in the kernel to verify that admitted ELF bytes match the manifest digest, and in the host `rustic-volume` provisioner before `seed7` writes the pair. `rustic-abi` remains free of a hashing dependency. These integrity checks do not authenticate a publisher or confer file authority.
