...
use rustic_sdk::{
    abi::files::{read::Request, reference::Version},
    files::{Client, Error as FileError},
};
...
impl State {
    ...
    fn load(&self, files: &mut Client, scope: u32) -> Result<Document, [u64; 8]> {
        let metadata = files.stat(scope).map_err(service)?;
        if metadata.directory {
            return Err(service(FileError::IsDirectory));
        }
        let references = files
            .references(u32::from(metadata.space), scope)
            .map_err(service)?;
        let expected_version = Version::new(metadata.version).map_err(service)?;
        ...
        let info = files
            .read_range(
                Request {
                    ...
                    offset: 0,
                    length: rustic_tasks_contract::MAX_BYTES as u16,
                },
                &mut bytes,
            )
            .map_err(service)?;
        if info.size > rustic_tasks_contract::MAX_BYTES as u64 {
            return Err(rustic_tasks_contract::wire::capacity());
        }
        if !info.eof || info.offset != 0 || info.size as usize != info.length {
            return Err(service(FileError::Protocol));
        }
        Document::parse(&bytes[..info.length]).map_err(document_error)
    }

    fn next_response(&mut self) -> [u64; 8] {
