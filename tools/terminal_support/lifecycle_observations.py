...
def inspect(uart, identity, state, inspector=None, stop=None, failure=None, completion=None):
    text = uart.command(f"inspect-operation {identity['id']}")
    ...
    if inspector is not None:
        client = act(uart, inspector, 'inspect', identity['id'])
...
def cancel(uart, identity, disposition, actor=None):
    if actor is None:
        text = uart.command(f"request-operation-cancel {identity['id']}")
    ...
    client = act(uart, actor, 'cancel', identity['id'])
