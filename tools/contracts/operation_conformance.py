...
def native_check(catalog, evidence):
    require(isinstance(evidence, dict) and evidence.get("verified") is True and evidence.get("boots") == 58, "incomplete native recovery mission")
    ...
    require(isinstance(cases, list) and len(cases) == 29 and all(isinstance(c, dict) and c.get("verified") is True for c in cases), "missing recovery cases")
